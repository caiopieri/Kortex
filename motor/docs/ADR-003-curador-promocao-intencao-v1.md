...
## Status
Aprovado para v1 pelo Arquiteto.
...
- `preparar_promocao_gated(...)` gera uma intenção de promoção com evidência.
- CLI e runbook operam essa trilha como fluxo read-only.
...
3. Gerar `status="promocao_pendente"` com `requer_gate=True` e evidência serializável.
4. Deixar a aplicação real de catálogo/config para um gate humano externo ao curador.
...
2. gate humano/organizacional explícito;
3. rollback testado;
4. histórico de promoção com before/after;
5. casos held-out certificados e congelados por slot;
6. evento `curador.promoveu` emitido somente após apply bem-sucedido.
