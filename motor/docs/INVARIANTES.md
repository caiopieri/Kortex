...
| S4 | Orçamento existe como contrato de spec e uso/custo é emitido, mas `teto_custo` **ainda não é hard-stop runtime**. | `motor/motor/spec.py::Restricoes.teto_custo`; `motor/motor/modelos.py` emite `modelo.uso` e `custo.tick` | ❌ NÃO SUSTENTADO — medição não impede chamada, retry ou failover. Correção planejada em `motor/specs/001-hardening-producao/plan-h12b.md` |
...
2. **S4 — orçamento como teto herdado:** implementar reserva/hard-stop antes de chamada, retry e failover conforme `motor/specs/001-hardening-producao/plan-h12b.md`; hoje há medição, não enforcement.
