...
**Versão:** 2 — substitui integralmente a v1 e o `interface-briefing.md` de 29/06 (02/07/2026). O que mudou: navegação passa a ser **por projeto** (seletor global); a Home vira cockpit de triagem; entram **Datahouse**, grafos **2D/3D**, **agentes personificados**, **skills** e as personas **Orquestrador/Curador**; a direção visual agora é explícita (centro de comando + control plane), em **dois temas** (claro/escuro). Do briefing de 29/06 ficam preservados: **zoom semântico** (§7.3), **motor headless / fronteira MCP** (§2) e **camadas atividade×conhecimento** (§8.8). O modelo de dados e eventos da v1 permanece (§5).
...
A direção é a de um **centro de comando e controle (C2)** — calma, precisão, human-in-the-loop — cruzada com a densidade de um **dev tool / control plane de agentes**. (Referências de inspiração, não de cópia nem de marca: dashboards de C2 militar moderno e control planes open-source de agentes.)

O que buscamos (conceito):
...
- **Tema claro:** off-white de papel, tipografia quase-preta em caps, hairlines, muito rigor. Clima: sala de briefing.
- **Tema escuro:** grafite quase-preto, texto off-white, telemetria brilhando. Clima: sala de operações à noite.
...
- **Canvases de grafo (2D, 3D, Mapa geral, Datahouse) permanecem escuros nos DOIS temas.** Pulso, brilho de conexão e sinal luminoso só funcionam sobre fundo escuro. No tema claro, o canvas escuro fica emoldurado pela UI clara: moldura limpa, teatro de operações escuro.
...
**Micro-detalhes de assinatura (o nível de acabamento):**
...
| Cor | Escuro | Claro | Forma/ícone (nunca só cor) | Significado |
|---|---|---|---|---|
...
| **Nó** | Unidade de trabalho no grafo (planejador, executor, verificador, **validador**, gate, síntese). | Nó-cartão no grafo 2D; ícone no 3D. |
...
| **Verificador** | Crítico **LLM adversarial**: julga a saída por rubrica (opinião de modelo; pode ser subjetivo). | Nó de crítica; peso visual "juízo". |
| **Validador (determinístico)** | Gate **verificável por máquina**: `schema_json` / `contem` (execução de teste/compile é um **nó ferramenta** via subprocess, não um kind de validador) → passa/falha por **algoritmo**, não por opinião. **É o diferencial vs. o gênero "agentic OS": Enforced Outcomes.** | Nó/ícone **distinto** do verificador. **Honestidade do selo:** o selo diz **o que** validou (ex.: "schema ok", "contém 5/7 termos"), não "verdade objetiva" genérica — hoje cobre sintaxe/presença, não correção/compilação. Emite `validador.rodou`. |
| **RAG / Contexto recuperado** | Um nó com `fonte_rag` consulta o Datahouse e recebe um bloco "CONTEXTO RECUPERADO" injetado antes de produzir — a **ponte Datahouse→workflow** (payoff do flywheel; lift de recuperação provado no medidor v3 — `LOG-VERIFICACAO.md` 2026-07-04). | Badge no nó ("consultou datahouse: N chunks") + aresta do dataset até o nó (ponte §6). Emite `rag.consultado`. |
...
Cada evento tem carimbo relativo `t` (segundos) e campo `evento`. **Fonte-da-verdade canônica: `motor/eventos_schema.py`** (48 tipos declarados hoje; o motor rejeita evento não-declarado — a lista abaixo é fiel a ele). A timeline e os grafos devem saber representar cada um. Agrupados por função:

**Ciclo de vida da missão**
- `spec.recebida` / `spec.criada` — missão entrou; traz `missao` e nº de `subagentes`.
- `rota.escolhida` — qual rota (`forja`/`construcao`/`pesquisa`/`hardware`).
- `paralelo.iniciado` / `paralelo.concluido` — fan-out abriu / fechou (lista de `subagentes`; quantos `commitados`).
- `tarefa.concluida` / `tarefa.abortada` — missão fechou / foi abortada.

**Roteamento de modelo** (o modelo concreto por chamada — ver §5.3)
- `modelo.pin` / `modelo.roteado_tier` — papel fixado / roteado para um `tier`.
- `modelo.roteado_capacidade` / `modelo.roteado_ferramentas` — roteado por capacidade exigida / por precisar de ferramentas (alimenta a Escada, §8.9).

**Execução**
- `executor.chamado` — executor começou; traz `executor`, `papel`, `tier`, `tentativa` **e o `modelo` concreto**.
- `executor.respondeu` / `executor.erro` — entregou / falhou (`motivo`, `tentativa`).
- `executor.escalado` — **subiu de tier/modelo após reprovação** (badge ↑). *(`escalado` é o alias legado; use `executor.escalado`.)*
- `modelo.uso` — consumo de tokens da chamada (alimenta custo/telemetria).
- `modelo.falha` — chamada de modelo falhou (`motivo`, `papel`, `tentativa`).

**Verificação e gates** — *distinguir os dois tipos:*
- `portao.aprovado` / `portao.reprovado` — gate decidiu; traz `portao`, `ciclo` e, se reprovou, `motivo` (**texto longo e técnico — o conteúdo mais valioso**; truncar com expandir).
- `validador.rodou` — **validador determinístico** (`schema_json`/`contem`; test/compile = nó ferramenta) rodou; passa/falha por algoritmo. **É o diferencial (Enforced Outcomes) — peso visual distinto do verificador LLM; o selo diz o que validou.**
- `gate.auto` — gate automático decidiu (`portao`, `decisao`).
- `juiz.independencia` — prova de que o juiz ≠ o produtor (sinal de integridade; badge discreto).

**Decisão humana** (alimentam a Caixa do Fundador, §8.2)
- `decisao.pendente` — abriu pendência humana.
- `decisao.plano` — gate de plano aguardando aval.
- `decisao.fundador` — o humano decidiu.
- `decisao.retomada` — run destravou após a decisão.
- `decisao.timeout` — **a decisão expirou** (o humano não respondeu a tempo) — estado a desenhar.

**Fase C — prevenção e reconciliação** (rota de dependência + conserto na fonte)
- `grafo_dep.iniciado` / `grafo_dep.travado` — rota de dependência começou / empacou (ciclo/impasse).
- `onda.iniciada` / `onda.concluida` — **ondas de dependência** (o grafo 2D de uma construção mostra as ondas).
- `reconciliacao.iniciada` / `reconciliacao.concluida` / `reconciliacao.esgotada` — reabriu os nós-raiz (badge ⟳ + sub-grafo destacado) / fechou / estourou o teto de rodadas.
- `lacuna.preenchida` — a reconciliação preencheu a inconsistência apontada.

**Superfície viva (viz)** — *o brief já desenha estes; agora têm nome:*
- `aresta.fluxo` — **a aresta animada** (`de`→`para`): é o "conexões acendem quando trafegam" (§8.6).
- `artefato.atualizou` — **a mini-janela de preview** do nó atualiza (§8.5) — dispara por evento, não frame a frame.
- `custo.tick` — **a linha de custo viva** pulsa (§8.1/§8.13).

**RAG / flywheel**
- `rag.consultado` — nó consultou o Datahouse (`fonte_rag`); traz nº de chunks. Badge no nó + aresta dataset→nó (§6).

**Ferramentas / MCP**
- `ferramenta.executada` / `ferramenta.saida_invalida` / `ferramenta.indisponivel` — MCP rodou / devolveu saída inválida / ausente.

**Resiliência (esgotamento 429)**
...
- `registro.sem_executor` — não havia executor registrado para o papel/tier.

### 5.3 Modelo/provedor por chamada — **disponível hoje** (não é mais lacuna)

A telemetria **já registra o modelo/provedor concreto por chamada** (`executor.chamado` carrega `modelo`; existe `modelo.uso` com tokens). **Desenhe como dado de primeira classe REAL**, não mock — badge de modelo/provedor no nó, eixo de análise no Curador, drill-down no Custos. Fallback gracioso "modelo: desconhecido" **só** para telemetria antiga (`sem-tier`/pré-instrumentação). O único progresso ainda **não** mensurável com honestidade é o "% contínuo de tarefa de LLM" — por isso o §8.6/§12 usam progresso-por-gate, não porcentagem.
...
agente/executor · planejador · verificador (crítico LLM) · **validador (determinístico — ícone distinto, "gate duro")** · gate · artefato · database · nota MD · dataset · workflow · projeto · skill · ferramenta/MCP.

> **Verificador vs. validador são entidades visuais diferentes.** O verificador é um juízo de LLM (pode discordar, é subjetivo); o validador é uma checagem de máquina (passou/falhou, objetivo). Dar o mesmo ícone aos dois apaga o nosso diferencial — o validador determinístico é a prova de que a fábrica **impõe** resultados, não só opina sobre eles.
...
FIXAS (não encolhem — o coração do painel)
  Home
  Board de missões           ← intake→planejamento→produção→concluída (§8.3a)
...
BIBLIOTECA ▾                 ← colapsável
  Workflows (catálogo)       ← templates versionados + autoria (§8.3b)
...
SISTEMA ▾                    ← colapsável
  ...
  Runners                    (máquinas de execução por capacidade + saúde; §8.15)
  Configurações
```
...
- Descrição da missão; **workflow/template** (do catálogo — ver §8.3b); **rota**; insumos (arquivo, caminho, spec); **preset de catálogo de modelos** (ex.: `free-escalada`, `codex`, `claude`); opções: `--auto` vs supervisionado, escalada on/off, reconciliação + teto de rodadas, gate de cobertura.
- **Escopo parcial (rodar só um pedaço):** ao escolher um template, o usuário pode **desmarcar nós** que não quer nesta missão (ex.: template de personagem, mas "só a voz" → desmarca corpo/aparência). A spec da missão vira um **subconjunto** do template.
- **Perfil MVP / rascunho:** um toggle "modo barato" que solta gates/validadores/escalada pra exploração. **Aviso honesto e visível:** o resultado sai **marcado "não-certificado"** (selo próprio) e **não entra na medição do Curador** — é rascunho, não pode se passar por verificado.
- **Runner (onde roda):** por padrão **automático** — o orquestrador escolhe um runner (§8.15) cujas capacidades cobrem o que a missão exige (ex.: build iOS → runner macOS; resto → Linux). O usuário pode **fixar** um runner. Se nenhum runner capaz estiver online, o disparo avisa (mesma lógica de "nenhum executor capaz" dos modelos), não falha no escuro.
- **Resumo "o que vai acontecer"** antes de confirmar (workflow + escopo + rota + catálogo + opções + selo de certificação) — escolhas custam tempo e dinheiro.
...
### 8.3b Catálogo de workflows + autoria (P1) — criar e versionar templates

A biblioteca dos **workflows (templates)** que as missões instanciam. Cada card: nome, "quando usar", **versão atual + histórico**, e a **evidência da versão** (aprovação de 1ª tentativa, custo, latência — do Curador). Versão nova = candidata; vira titular só com evidência que bate a anterior.

- **Criar workflow novo (autoria-como-run):** botão "Novo workflow" abre um **chat com o Orquestrador**. Ele propõe uma **pesquisa de mercado** (padrão de indústria), arquiteta, roda, **sintetiza e devolve um rascunho de WorkflowSpec** — é uma **run do motor** como qualquer outra (aparece no grafo, tem digest). O rascunho abre no **editor de workflow** (§8.5) pra ajuste antes de catalogar.
- **Editar/versionar:** alterar um template gera **nova versão** (reversível). Quando o usuário propõe uma mudança, o Orquestrador pode dizer "acho que engasga por X; **rodo em sombra** pra confirmar" — e **a decisão melhor/pior vem do número da sombra, não da opinião** (o selo "certificado por sombra" aparece na versão). Ver `DECISAO-ciclo-de-vida-workflow.md`.
- **Melhorias do Curador:** propostas de mudança de workflow (com evidência) entram na **Caixa do Fundador** (§8.2), não se aplicam sozinhas.

### 8.3a Board de missões — intake e ciclo de vida (P0; a porta de entrada)

O estágio **antes** do disparo: capturar ideia crua, amadurecer no planejamento, empurrar pra produção, ver concluída. Um **kanban** cujas colunas são o **ciclo de vida da missão** — não é um gestor de projetos genérico, é uma **vista sobre o estado real** (cada card lastreado nos mesmos eventos/estado do motor, nunca um banco paralelo). É uma forma natural de Home pra quem chega com ideias soltas (aba irmã do §8.1).

**Colunas (o ciclo de vida):**

1. **Ideias** — backlog cru. Card = *candidato* a missão/projeto; título + nota curta, arrastável. Escopo: só ideia **fabricável** (nota de vida genérica é do Flint, projeto externo — não mora aqui).
2. **Planejamento** — promover um card abre o **chat com o Planejador**, que co-autora a **WorkflowSpec** com você (a "receita"): subtarefas, rubricas, gates, dependências. É o **gate de plano** que já existe no motor (evento `decisao.plano`) virando tela. Abre no **modo edição do grafo** (§8.5) pra montar/ajustar o workflow antes de rodar.
3. **Produção** — "empurrar" dispara `despachar_missao`; o card passa a espelhar o run (status §4).
4. **Precisa de você** — **coluna própria** (não é detalhe escondido): quando um gate humano dispara (aprovar plano, decidir reprovação cara), o card vem pra cá em **âmbar**. Correção honesta ao "empurro e só vejo pronto": os gates do fundador te puxam de volta — o board mostra isso, não finge autonomia total. Clique = abre a Caixa do Fundador (§8.2) naquele item.
5. **Concluída** — o **digest** do run; verde. Arquivável.

**Regras:** card mostra bolinha+forma do §4 e o último acontecimento (mesma string do último evento/digest). Mover card de "Ideias"→"Planejamento" é a única transição que o humano arrasta; da Produção em diante quem move é o **motor** (o board reflete o estado, não comanda por arrasto). Um card de "Ideias" pode virar **projeto** (várias missões) ou **missão** única — a promoção decide.
...
Canvas de **nós-cartão** (estilo React Flow / node-graph), topologia planejador → fan-out → verificadores → gate → síntese.
...
**Dois tipos de gate no grafo (não confundir):** o **verificador** (crítico LLM, nó de "juízo") e o **validador determinístico** (nó/ícone distinto — `validador.rodou`, passa/falha objetivo). O validador é o nosso diferencial; deve ler-se como o portão duro, visualmente separado do palpite do LLM.

**Consumo de Datahouse (RAG) visível:** um nó com `fonte_rag` mostra badge "consultou datahouse: N chunks" (`rag.consultado`) e uma **aresta vinda do dataset** (ponte §6) — é o flywheel em ação, o motivo de a Datahouse existir.

**Momentos críticos com destaque:** gate reprovado, escalada (`executor.escalado`), reconciliação (`reconciliacao.*` — mostrar o sub-grafo reaberto e a ordem de dependência do refazimento), ondas de dependência (`onda.*`). Pendência humana = impossível de ignorar, com atalho para a Caixa do Fundador; decisão expirada (`decisao.timeout`) é um estado próprio.

**Modo edição / autoria (o editor de workflow — abre do "Planejamento", §8.3a).** A mesma tela do grafo, alternável entre **ver** (monitorar o run vivo) e **editar** (montar/personalizar o workflow antes de rodar). Editar aqui **é editar a WorkflowSpec por baixo** — o grafo é a representação, o dado é a verdade. O usuário pode:

- **Adicionar/remover agentes** (nós `modelo`): definir papel, tier, capacidades, modelo fixado, rubrica, `fonte_rag`.
- **Ligar/desligar dependências** — arrastar uma aresta declara `depende_de`; o motor reordena em ondas. Inserir um agente "no meio" = criá-lo e religar as dependências à volta dele.
- **Inserir um validador determinístico** (`schema_json`/`contem`) entre produtor e consumidor — o gate duro.
- **Inserir um gate do fundador** (condição + pergunta + opções).

**Trava de arquitetura (importante pro editor não mentir):** a **topologia** é limitada a **dois padrões certificados** — `fan_out_sintese` (paralelo) e `grafo_dependencias` (um DAG). Dentro do DAG a liberdade é grande (qualquer grafo **acíclico** de tarefas + validadores + gates), mas **não** é um canvas de fios livres tipo n8n: o editor oferece só a gramática válida (adicionar subtarefa/dependência/validador/gate), **valida ao vivo** contra o schema (rejeita ciclo, dependência quebrada, validador que se auto-valida, nó sem rubrica) e **bloqueia o disparo de spec inválida**. Um *padrão novo* (topologia que não é nenhum dos dois) não se desenha aqui — entra como versão de spec certificada (raro). É isso que preserva as garantias do motor (verificação, cobertura, reconciliação) enquanto deixa você e o mantenedor personalizarem tudo com segurança. Hoje a spec se edita como JSON (`exemplos/*.json`); este editor visual é a evolução P1/P2 dessa mesma edição.
...
- **Runners / ambiente de execução:** a lista das **máquinas onde o trabalho de fato roda** (não onde o motor pensa — o motor é leve e roda 24/7 no plano de controle; runners são o músculo). Cada runner é uma entidade com **capacidades** (SO=linux/macos/windows, ferramentas=xcode/kicad/docker/gpu…) + status de saúde (online/ocupado/offline) + fila. Ex.: um runner **macOS** (o Mac do Caio) pro que exige Apple/iOS; runners **Linux** pro resto; caixa de **GPU** pro pesado. É o **roteamento por capacidade dos modelos, generalizado pra máquinas** — mesmo ícone/telemetria. Um runner é só mais um tipo de entidade do catálogo (§8.3b/Registry).
- **Configurações:** **tema claro / escuro** — com atalho no topbar, persistido; densidade; **animações on/off** (performance); limites de stream; retenção de logs.
...
- **Não é gestor de projetos genérico** (Notion/Linear): o Board de missões (§8.3a) é uma *vista sobre o ciclo de vida real* das missões, não um kanban com regras de workflow customizáveis, WIP limits e banco próprio.
- **Não é um construtor de fluxo livre** (n8n/ComfyUI): o editor de workflow (§8.5) edita a WorkflowSpec dentro de dois padrões certificados; topologia arbitrária não se desenha aqui.
...
5. **Home em alta fidelidade** nos estados: saudável, com pendências, com erro, vazia — **nos dois temas** (claro e escuro); as demais telas podem ser entregues num tema só, desde que usem apenas tokens semânticos do §3.1.
...
**Critério de aceite da estética:** a tela lê como um centro de comando calmo e preciso (mesma família: calma, precisão, controle) — e nada nela viola os anti-padrões do §3.1.
...
> 4. A sensação-alvo é a de um C2 militar moderno: calma, precisão, tudo sob controle. Se uma tela parecer dashboard SaaS colorido, está errada.
